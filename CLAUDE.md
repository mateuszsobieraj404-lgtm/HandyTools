...
- Pomocnik Remote (`src/tools/pomocnik*`, `src/konto.js`): podgląd bazy Supabase aplikacji Pomocnik List Sprzętu (osobne repo `pomocnik-list-sprzetu`), TYLKO ODCZYT, pilnuje tego `src/tools/pomocnik/tylko-odczyt.test.js`. Adres bazy i klucz wpisuje użytkownik w Koncie; nigdy nie wpisuj ich do repo.
- Testy: `npm test`.
